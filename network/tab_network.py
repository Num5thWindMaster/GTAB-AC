...
from network.GNN_network import GATGCN


# from .GNN_network import GAT_GCN  # server
...
class TabNetEncoder(torch.nn.Module):
    def __init__(
        ...
        if self.group_attention_matrix is None:
            # no groups
            self.group_attention_matrix = torch.eye(self.input_dim).to(
...
class TabNetNoEmbeddings(torch.nn.Module):
    def __init__(
            ...
            graph_output_dim=0,
            graph_input_dim=768,
            graph_pe_encoding=None,
            graph_branch_only=False
    ):
        """
        Defines main part of the TabNet network without the embedding layers.
        ...
        self.graph_pe_encoding = graph_pe_encoding
        self.graph_output_dim = graph_output_dim
        self.graph_branch_only = graph_branch_only
        ...
        if self.is_multi_task:
            ...
            if self.graph_branch:
                self.graph = GATGCN(output_dim=self.graph_output_dim,
                                    # num_features_xnode=self.graph_input_dim + self.graph_pe_encoding if self.graph_pe_encoding else self.graph_input_dim)
                                    num_features_xnode=self.graph_input_dim)
                self.relu = ReLU()
                self.dropout = Dropout(p=0.2)
                if self.graph_branch_only:
                    self.final_mapping = Linear(self.graph_output_dim, output_dim, bias=False)
                else:
                    self.final_mapping = Linear(n_d + self.graph_output_dim, output_dim, bias=False)
    ...
    def forward(self, x):
        if self.graph_branch:  # +
            xg = self.graph.forward(x)  # +
        x = x.abundance.reshape(-1, self.input_dim)
        res = 0
        if not self.graph_branch_only:
        ...
        if self.is_multi_task:
            ...
            if not self.graph_branch:
                out = self.final_mapping(res)
                return out, M_loss
            else:  # +
                if not self.graph_branch_only:
                    res = torch.concat((res, xg), dim=1)
                    out = self.final_mapping(res)
                    return out, M_loss
                else:
                    res = xg
    ...
    def forward_masks(self, x):
        if self.graph_branch_only:
            raise NotImplementedError('Graph branch only not support tabnet masks.')
        x = x.abundance.reshape(-1, self.input_dim)
...
class TabNet(torch.nn.Module):
    def __init__(
        ...
        self.tabnet = TabNetNoEmbeddings(
            ...
            graph_branch_only=graph_branch_only
        )
...
class EmbeddingGenerator(torch.nn.Module):
    ...
    def __init__(self, input_dim, cat_dims, cat_idxs, cat_emb_dims, group_matrix):
        ...
        if cat_dims == [] and cat_idxs == []:
            ...
            self.embedding_group_matrix = None
            return
        else:
